...
def overall(mo, mv, so, sv, smv, filename, fig_files=None, plot_title=None, n_fig=None, plot_init_in=False):
    ...
    if so and smv:
        ...
        if mo.voc_soc is not None:
            ...
            plt.plot(mo.time, mo.Voc, color='blue', linestyle='-.', label='Voc')
            plt.plot(mo.time, mo.voc, color='red', linestyle=':', label='voc')
            plt.plot(mv.time, mv.voc, marker='+', markersize='3', markevery=8, linestyle='None', color='black', label='voc_ver')
    ...
    plt.subplot(224)
    plt.plot(so.time, so.ib_s, linestyle='-', color='black', label='ib_s')
    plt.plot(smv.time, smv.ib_s, linestyle='--', color='cyan', label='ib_s_ver')
