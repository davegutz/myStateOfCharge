...
def overall(mo, mv, so, sv, smv, filename, fig_files=None, plot_title=None, n_fig=None, plot_init_in=False,
    ...
    plt.plot(so.time, so.voc_stat_s, color='cyan', linestyle='--', label='voc_stat_s'+old_str)
    plt.plot(smv.time, smv.voc_stat_s, color='magenta', linestyle='-.', label='voc_stat_s'+new_str)
    plt.legend(loc=2)
    plt.subplot(332)
    plt.plot(mo.time, mo.dv_hys, color='black', linestyle='-', label='dv_hys'+old_str)
    plt.plot(mv.time, mv.dv_hys, color='orange', linestyle='--', label='dv_hys'+new_str)
    plt.plot(mo.time, mo.dv_dyn, color='blue', linestyle='-.', label='dv_dyn'+old_str)
    plt.plot(mv.time, mv.dv_dyn, color='red', linestyle=':', label='dv_dyn'+new_str)
    # plt.plot(mo.time, mo.ioc, color='black', linestyle='-', label='ioc'+old_str)
    # plt.plot(mv.time, mv.ioc, color='orange', linestyle='--', label='ioc'+new_str)
    plt.legend(loc=3)
    plt.subplot(333)
    plt.plot(mo.time, mo.soc, linestyle='-', color='black', label='soc'+old_str)
    plt.plot(mv.time, mv.soc, linestyle='--', color='orange', label='soc'+new_str)
    plt.plot(so.time, so.soc_s, linestyle='-.', color='blue', label='soc_s'+old_str)
    plt.plot(sv.time, smv.soc_s, linestyle=':', color='red', label='soc_s'+new_str)
    plt.plot(mo.time, mo.soc_ekf, linestyle='-', color='cyan', label='soc_ekf'+old_str)
    plt.plot(mv.time, mv.soc_ekf, linestyle='--', color='magenta', label='soc_ekf'+new_str)
    plt.legend(loc=4)
    plt.subplot(334)
    plt.plot(mo.time, mo.ib_sel, linestyle='-', color='black', label='ib_sel'+old_str)
    plt.plot(so.time, so.ib_s, linestyle='--', color='cyan', label='ib_in_s'+old_str)
    ...
    plt.legend(loc=3)
    plt.subplot(335)
    plt.plot(mo.time, mo.voc, linestyle='-', color='black', label='voc'+old_str)
    plt.plot(mv.time, mv.voc, linestyle='--', color='orange', label='voc'+new_str)
    ...
    plt.plot(mo.time, mo.vsat, linestyle='-', color='cyan', label='vsat'+old_str)
    plt.plot(mv.time, mv.vsat, linestyle='--', color='magenta', label='vsat'+new_str)
    plt.legend(loc=2)
    plt.subplot(336)
    ...
    # plt.plot(mv.time, np.array(mv.voc_soc) - np.array(mv.voc_stat), linestyle='--', color='orange', label='voc_soc - voc_stat '+new_str)
    # plt.plot(mv.time, np.array(mv.voc_soc) - np.array(mv.voc_stat), linestyle=':', color='blue', label='voc_soc - voc_stat '+new_str)
    plt.legend(loc=2)
    plt.subplot(337)
    # plt.plot(mo.soc, mo.voc, linestyle='-', color='black', label='voc'+old_str)
    # plt.plot(mv.soc, mv.voc, linestyle='--', color='orange', label='voc'+new_str)
    # plt.plot(mo.soc, mo.voc_stat, linestyle='-.', color='blue', label='voc_stat'+old_str)
    # plt.plot(mv.soc, mv.voc_stat, linestyle=':', color='red', label='voc_stat'+new_str)
    # plt.plot(mo.soc, mo.vsat, linestyle='-', color='cyan', label='vsat'+old_str)
    # plt.plot(mv.soc, mv.vsat, linestyle='--', color='magenta', label='vsat'+new_str)
    plt.plot(mo.soc, mo.vb, color='cyan', linestyle='-', label='vb'+old_str)
    plt.plot(so.soc, so.vb_s, color='magenta', linestyle='--', label='vb_s'+old_str)
    plt.plot(smv.soc, smv.vb_s, color='black', linestyle='-.', label='vb_s'+new_str)
    plt.legend(loc=2)
    plt.subplot(338)
    # plt.plot(so.time, so.voc_stat_s, color='black', linestyle='-', label='voc_stat_s'+old_str)
    # plt.plot(smv.time, smv.voc_stat_s, color='orange', linestyle='--', label='voc_stat_s'+new_str)
    # plt.plot(so.time, so.voc_s, color='blue', linestyle='-.', label='voc_s'+old_str)
    # plt.plot(smv.time, smv.voc_s, color='red', linestyle=':', label='voc_s'+new_str)
    plt.plot(mo.time, mo.vb, color='cyan', linestyle='-', label='vb'+old_str)
    plt.plot(so.time, so.vb_s, color='magenta', linestyle='--', label='vb_s'+old_str)
    plt.plot(smv.time, smv.vb_s, color='black', linestyle='-.', label='vb_s'+new_str)
    plt.legend(loc=2)
    plt.subplot(339)
    plt.plot(mo.time, mo.dv_hys, color='black', linestyle='-', label='dv_hys'+old_str)
    plt.plot(mv.time, mv.dv_hys, color='orange', linestyle='--', label='dv_hys'+new_str)
    plt.plot(so.time, so.dv_hys_s, color='blue', linestyle='-.', label='dv_hys_s'+old_str)
    plt.plot(smv.time, smv.dv_hys_s, color='red', linestyle=':', label='dv_hys_s'+new_str)
    plt.legend(loc=3)
    fig_file_name = filename + '_' + str(n_fig) + ".png"
    fig_files.append(fig_file_name)
    plt.savefig(fig_file_name, format="png")
