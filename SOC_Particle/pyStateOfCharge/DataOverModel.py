...
def dom_plot(mo, mv, so, sv, smv, filename, fig_files=None, plot_title=None, fig_list=None, plot_init_in=False,
    ...
    if mo.ibmh is not None and hasattr(mo, 'ib_diff_f'):
        ...
        plq(plt, mo, 'time', mo, 'ib_sel', add=+1, color='red', linestyle='-.', label='ib_sel'+ref_str+'+1')
        plq(plt, mo, 'time', mo, 'ib_charge', add=+1, linestyle=':', color='blue', label='ib_charge'+ref_str+'+1')
        plt.legend(loc=1)
        plt.subplot(222)
        plq(plt, mo, 'time', mo, 'ib_sel_stat', color='black', linestyle='-', label='ib_sel_stat'+ref_str)
        plq(plt, mo, 'time', mo, 'ib_dec', add=2, color='red', linestyle='--', label='ib_dec'+ref_str+'+2')
        plt.legend(loc=1)
        plt.subplot(223)
