...
lookup = {'init': ('Y;cc;Dh1800000;*W;*vv0;*XS;*Ca1;<HR;<Rf;<Hd;<Pf;', ('',), 10, 12),
          'end_early': ('Y;cc;Dh1800000;*W;*vv0;*XS;*Ca1;<Hd;<Pf;', ('',), 10, 12),
          'initMid': ('Y;cc;Dh1800000;*W;*vv0;*XS;*Ca.5;<HR;<Rf;<Hd;<Pf;<XD;', ('',), 10, 12),
          ...
          'ampHiFail': ('Ff0;HR;Xm247;Ca0.5;DP1;Pf;vv2;Dh1000;W20;Dm50;Dn0.0001;XQ40000;Dh;', ("Should detect and switch amp current failure (reset when current display changes from '50/diff' back to normal '0' and wait for CoolTerm to stop streaming.)", "'diff' will be displayed. After a bit more, current display will change to 0.", "To evaluate plots, start looking at 'DOM 1' fig 3. Fault record (frozen). Will see 'diff' flashing on OLED even after fault cleared automatically (lost redundancy).", "ib_diff_fa will set red_loss but wait for wrap_fa to isolate and make selection change"), 1, 62),
          'rapidTweakRegression': ('Ff0;HR;Pf;Xp10;Dh;', ('Should run three very large current discharge/recharge cycles without fault', 'Best test for seeing time skews and checking fault logic for false trips'), 1, 210),
          'allProto': ('Ff0;HR;Xm247;Ca0.5;DP1;Pf;vv4;Dh1000;W20;Dm50;Dn0.0001;XQ50000;W5;Rf;  Pf;Xp10;  Pf;Xp13;  <Dh;', ('Proto multi', "Must have same 'vv*' throughout", "No 'HR' either"), 1, 480),
          'pulseSS': ("Xp7;", ("Should generate a very short <10 sec data burst with a sw pulse.  Look at plots for good overlay. e_wrap will have a delay.", "This is the shortest of all tests.  Useful for quick checks."), 1, 18),
          'rapidTweakRegressionH0': ('Sh0;SH0;Ff0;HR;Pf;Xp10;Dh;', ('Should run three very large current discharge/recharge cycles without fault', 'No hysteresis. Best test for seeing time skews and checking fault logic for false trips', 'Tease out cause of e_wrap faults.  e_wrap MUST be flat!'), 1, 12),
          ...
          'triTweakDisch': ('Ff0;HR;Pf;Xp13;Dh;', ('Should run three very large current discharge/recharge cycles without fault', 'Best test for seeing time skews and checking fault logic for false trips'), 1, 12),
          ...
          'slowTweakRegression': ('Ff0;HR;Xp11;', ("Should run one very large slow (~15 min) current discharge/recharge cycle without fault.   It will take 60 seconds to start changing current.",), 1, 12),
...
if __name__ == '__main__':
    ...
    end_early_val, dum1, dum2, dum3 = lookup.get('end_early')
