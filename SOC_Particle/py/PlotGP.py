...
def gp_plot(mo, mv, so, sv, smv, filename, fig_files=None, plot_title=None, n_fig=None, plot_init_in=False,
    ...
    plt.plot(mv.soc, np.array(mv.voc_soc) - np.array(mv.voc)+13., color='blue', linestyle='-', label='dv' + test_str + '+13')
    plt.legend(loc=1)
    plt.subplot(326)
    plt.plot(mo.time, mo.voc, color='black', linestyle='-', label='voc' + ref_str)
    ...
    # plt.plot(mv.soc, mv.voc_soc_new, color='magenta', linestyle='-.', label='voc_soc_new' + test_str)
    plt.plot(mv.time, np.array(mv.voc_soc) - np.array(mv.voc)+13., color='blue', linestyle='-', label='dv' + test_str + '+13')
    plt.legend(loc=1)
    fig_file_name = filename + '_' + str(n_fig) + ".png"
    fig_files.append(fig_file_name)
    plt.savefig(fig_file_name, format="png")
    ...
    plt.plot(mo.soc, smv.vb_s, color='cyan', linestyle='--', label='vb_s' + test_str)
    plt.plot(mo.soc, mo.voc_stat, color='orange', linestyle='-.', label='voc_stat' + ref_str)
    plt.plot(mo.soc, smv.voc_stat_s, color='red', linestyle=':', label='voc_stat_s' + test_str)
