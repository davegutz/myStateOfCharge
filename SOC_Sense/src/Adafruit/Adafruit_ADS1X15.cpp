...
int16_t Adafruit_ADS1X15::readADC_SingleEnded(uint8_t channel) {
  ...
  while ( !conversionComplete() && ++count<count_max )
    ;
  if ( count==count_max && sp.debug_z>0 && !sp.modeling_z ) Serial.printf("WARNING(readADC_SingleEnded):  timed out hardcoded count limit**********************\n");
...
int16_t Adafruit_ADS1X15::readADC_Differential_0_1(const String name) {
  ...
  if ( count==count_max )
  {
    if( ++yaks==1 && sp.debug_z>0 ) Serial.printf("WARNING(readADC_Differential_0_1)%s:  timed out hardcoded count limit**********************\n", name.c_str());
  }
  if ( yaks >= 50 )  yaks = 0;
  // Read the conversion results
  return getLastConversionResults();
...
int16_t Adafruit_ADS1X15::readADC_Differential_2_3(const String name) {
  ...
  while ( !conversionComplete() && ++count<count_max )
    ;
  if ( count==count_max && sp.debug_z>0 ) Serial.printf("WARNING(readADC_Differential_2_3)%s:  timed out hardcoded count limit**********************\n", name.c_str());
  if ( sp.debug_z==-2 ) Serial.printf("count for ADS%s:  %d\n", name.c_str(), count);
